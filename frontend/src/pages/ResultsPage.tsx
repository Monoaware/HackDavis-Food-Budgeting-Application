...
function MealCarousel({ meals }: { meals: Meal[] }) {
  const [index, setIndex] = useState(0)
  const withImages = meals.filter(m => m.image)
  if (withImages.length === 0) return null

  const meal = withImages[index]

  return (
    <div className="meal-carousel">
      <img className="meal-carousel-img" src={meal.image} alt={meal.title} />
      <div className="meal-carousel-overlay">
        <span className="meal-carousel-title">{meal.title}</span>
        {withImages.length > 1 && (
      ...
      {withImages.length > 1 && (
        <>
          <button
            ...
            onClick={() => setIndex(i => (i - 1 + withImages.length) % withImages.length)}
          ...
          <button
            ...
            onClick={() => setIndex(i => (i + 1) % withImages.length)}
